...
def send_to_kindroid(prompt):
    ...
    try:
        ...
        if response.status_code == 401:
            print("[Kindroid ERROR: 401 Unauthorized] Check KINDROID_API_KEY or AI ID")
            return "Authorization failed. Please check credentials."

        elif response.status_code == 404:
            print(f"[Kindroid ERROR: 404] URL not found: {url}")
            return "The Kindroid API endpoint was not found."

        elif response.status_code != 200:
            print(f"[Kindroid ERROR: {response.status_code}] {response.text[:300]}")
            return "Kindroid could not respond. Try again later."

        try:
            result = response.json()
            return result.get("reply", "No reply from Kindroid.")
        except ValueError:
            print(f"[Kindroid WARNING] Non-JSON reply: {response.text[:300]}")
            return response.text[:300] or "Kindroid returned a non-JSON response."

    except requests.exceptions.RequestException as e:
        print(f"[Request Error] {e}")
        return "Network error while contacting Kindroid."
